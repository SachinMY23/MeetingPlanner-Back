...
//start of recoverPassword function
...
//start of deleteAllUsers function
//function to delete all the users
let deleteAllUsers = (req, res) => {
    userModel.remove({'isAdmin':false}).exec((err, result) => {
        if (err) {
            ...
            let apiResponse = response.generate(false, 'Users Deleted Successfully', 200, result)
            res.send(apiResponse)
        }
    })
}//end of deleteAllUsers function

//Start of deleteSingeUser Function
//This function deletes the record of a single user
...
}//End of delete singleUser Function

//start of editProfile function
//This function edits the user profile information
let editProfile=(req,res)=>{
    if (check.isEmpty(req.params.userId)) {
        ...
        userModel.updateOne({ userId: req.params.userId }, options, { multi: true }).exec((err, result) => {
            ...
            if (err) {
                ...
                let apiResponse = response.generate(false, 'User profile Edited Successfully.', 200, result)
                res.send(apiResponse)
            }
        })
    }
...
//start of getSingleUser function
//this functions get the details of a single user
